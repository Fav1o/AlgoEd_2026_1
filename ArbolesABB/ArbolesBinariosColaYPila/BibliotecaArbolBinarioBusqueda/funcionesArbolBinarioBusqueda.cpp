...
//Funciones Nuevas que No Darán en el Lab
void llenarArbol(ArbolBinarioBusqueda &arbol,ElementoArbolBinarioBusqueda *arreglo,int inicio,int fin) {
    llenarArbolRecursivo(arbol.raiz,arreglo,inicio,fin);
}

void llenarArbolRecursivo(NodoArbolBinarioBusqueda *&nodo,ElementoArbolBinarioBusqueda *arreglo,
                        int inicio,int fin) {
    // estrategia de divide y venceras
    if (inicio >= fin) return;
    int medio = (inicio + fin) / 2;
    plantarNodoArbolBinario(nodo,nullptr,arreglo[medio],nullptr);
    llenarArbolRecursivo(nodo->izquierda,arreglo,inicio,medio);
    llenarArbolRecursivo(nodo->derecha,arreglo,medio+1,fin);
}

void enOrdenIterativo(const struct ArbolBinarioBusqueda &arbol) {
    struct Pila pila;
    construirPila(pila);
    struct NodoArbolBinarioBusqueda * actual = arbol.raiz;

    while (not esNodoVacio(actual) or not esPilaVacia(pila) ) {
